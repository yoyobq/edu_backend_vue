  ...
  async index(params) {
    ...
    if (params.uId !== undefined) {
      ...
      if (params.type !== undefined) {
        ...
        // console.log(params);
        moduleApplyRecords = await this.app.mysql.query('SELECT a.`id`, a.`uId`, a.`submitTime`, a.`ALA`, a.`comments`, b.`realName`, b.`school`, b.`email`, b.`cellphone`, b.`department`, b.`highestDegree`, a.`type`, a.`status` FROM `ta_module_apply_records` a LEFT JOIN `pf2_accounts_informations` b ON (a.`uId` = b.`id`) WHERE a.`modId` = ?', [ parseInt(params.modId) ]);
      }
    }
